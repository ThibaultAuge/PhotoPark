...
export function BodyFiltersBar({ filters, setFilters, referenceData, onReset }: { filters: BodyFilters; setFilters: (filters: BodyFilters) => void; referenceData: BodyReferenceData; onReset: () => void }) {
  return (
    <CollapsibleFilters onReset={onReset}>
      ...
      <label>Type<select value={filters.bodyType} onChange={(event) => setFilters({ ...filters, bodyType: event.target.value as BodyFilters["bodyType"] })}><option value="">Tous</option><option value="mirrorless">Hybride</option><option value="dslr">Reflex</option><option value="compact">Compact</option></select></label>
