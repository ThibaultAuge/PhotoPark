  ...
  return (
    <div className="filters card">
      ...
      <div className="filter-sliders">
        <DualRangeSlider label="Focale min" min={0} max={1000} low={filters.focalMinLow} high={filters.focalMinHigh} step={1} formatValue={(v) => `${v} mm`} onChange={(range) => setFilters({ ...filters, focalMinLow: range.low, focalMinHigh: range.high })} />
        <DualRangeSlider label="Focale max" min={0} max={1000} low={filters.focalMaxLow} high={filters.focalMaxHigh} step={1} formatValue={(v) => `${v} mm`} onChange={(range) => setFilters({ ...filters, focalMaxLow: range.low, focalMaxHigh: range.high })} />
