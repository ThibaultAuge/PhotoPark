...
export function LensCard({ lens, selected, onToggleSelected, onEdit }: { lens: Lens; selected: boolean; onToggleSelected: (id: string) => void; onEdit: (lens: Lens) => void }) {
  return (
    <article className={`card lens-card ${selected ? "selected" : ""}`}>
      ...
      <dl><div><dt>Type</dt><dd>{getLensKind(lens)}</dd></div><div><dt>Monture</dt><dd>{lens.mount}</dd></div><div><dt>Focale</dt><dd>{formatFocalRange(lens)}</dd></div><div><dt>Ouverture</dt><dd>{formatApertureRange(lens)}</dd></div><div><dt>Prix</dt><dd>{lens.priceEur ? `${lens.priceEur} €` : "—"}</dd></div></dl>
